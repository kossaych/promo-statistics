...
fichier_excel = "moy.xlsx"
fichier_excel = os.path.join(os.path.dirname(__file__), fichier_excel)
NON_SUBJECT_COLUMNS = ["mgp","résultat",'N°','Ident.','Nom','Prenom']
...
def get_students():
    ...
    tab1  = pd.concat(tableaux[:3], ignore_index=True)
    tab2  = pd.concat(tableaux[3:6], ignore_index=True)
    tab3 = pd.concat(tableaux[6:9], ignore_index=True)
    tab4  = pd.concat(tableaux[9:12], ignore_index=True)
    dfs = [tab1, tab2, tab3, tab4]
    tab = reduce(lambda left, right: pd.merge(left, right, on=IN_ALL_TABLES_COLUMNS, how='inner'), dfs)
    ##########################################################################################################

    file_path = "coefficients.pkl"
    ...
    for etudiant in etudiants :
        ...
        ML = (etudiant['Anglais 1'] + etudiant['Tech Com 2'] + etudiant['Tech com 1'] + etudiant['Anglais 2'])/4 #score des langues
        etudiant['ML']  = round(ML,2)
        
        etudiant['A'] = round(0.2 * M + 1.4 / 3 * R + 5 / 6 * ML + 2 / 3 * (B1 + B2),2)
        etudiant['E'] = round((etudiant['Analyse 1'] + etudiant['Algèbre 2'] + etudiant['Algèbre 1']  +etudiant['Analyse 2'] )/4 + (etudiant['Alg&Struct donnée'] + etudiant['Algo&Struct Donnée1'])/2 ,2)
    ...
    return etudiants
def get_subjects():
    # Returns a list of subjects from the coefficients dictionary.
    file_path = "coefficients.pkl"
    file_path = os.path.join(os.path.dirname(__file__), file_path)
    if os.path.exists(file_path):
        with open(file_path, "rb") as f:
